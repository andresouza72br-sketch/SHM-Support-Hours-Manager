...
7. **Motor Dual-Engine de Geração de Documentos (Extrato Oficial em PDF Vetorial):** O serviço `ExtratoPdfService` implementa compilação server-side de alta fidelidade com renderizador primário `WeasyPrint` (CSS Paged Media `@page` com contadores dinâmicos de página e cabeçalhos repetidos) e fallback automático para o motor nativo `ReportLab Platypus`. Cada binário gerado tem seu hash SHA-256 estampado no rodapé pericial e gravado em `ExtratoOficialGerado`, com armazenamento no storage híbrido da VPS e espelhamento no Google Drive.
8. **Parametrização Institucional Centralizada (Branding Corporativo Singleton):** O modelo `ConfiguracaoBranding` (instância única `id=1`) gerencia os dados cadastrais, fiscais, logotipo corporativo e assinatura/rubrica digitalizada do responsável técnico com teto de upload de 5.0 MB e validação por Pillow. Esses dados alimentam dinamicamente os cabeçalhos de relatórios, rodapés de e-mails e elementos visuais da SPA.
